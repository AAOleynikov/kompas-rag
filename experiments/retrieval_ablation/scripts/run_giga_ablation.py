"""Ablation study: Giga-Embeddings (12 конфигураций) для сравнения с E5."""
...
def main():
    ...
    parser.add_argument("--giga-chroma", default="/storage/chroma_db_giga_backup")
    parser.add_argument("--persist-dir", default=None, help="Alias for --giga-chroma.")
    parser.add_argument("--bm25-index-path", default=None, help="Override persisted BM25 index path.")
    parser.add_argument("--collection", default="kompas_docs")
    ...
    parser.add_argument("--results-output", default="/storage/ablation_giga_results.json")
    parser.add_argument("--api-key", required=True)
    parser.add_argument("--api-base", required=True)
    parser.add_argument("--model", default="tgpt/qwen3-coder-480b-a35b-instruct")
    args = parser.parse_args()
    if args.persist_dir:
        args.giga_chroma = args.persist_dir
    ...
    giga_emb = HuggingFaceEmbeddings(
        model_name="ai-sage/Giga-Embeddings-instruct",
        model_kwargs={"device": "cuda", "trust_remote_code": True},
        encode_kwargs={"normalize_embeddings": True, "batch_size": 64},
    )
    test = giga_emb.embed_query("тест")
    logger.info(f"Giga dim={len(test)}")

    giga_vs = Chroma(
        collection_name=args.collection,
        persist_directory=args.giga_chroma,
        embedding_function=giga_emb,
    )
    logger.info(f"Giga Chroma: {get_vectorstore_count(giga_vs)} docs")

    # BM25 загружается из сохраненного индекса, построенного на этапе ingest.
    config = load_config(args.config)
    if args.bm25_index_path:
        config.retriever.bm25_index_path = args.bm25_index_path
    bm25_index = load_bm25_index(
        config.retriever.bm25_index_path,
        expected_doc_count=get_vectorstore_count(giga_vs),
    )
    ...
    print()
    print("=" * 100)
    print(f"  ABLATION STUDY — Giga-Embeddings — {len(annotated_qi)} запросов, {len(annotations)} аннотаций")
    print("=" * 100)
    ...
    for cfg_name, m in all_results.items():
        group = cfg_name.split(".")[1].strip().split()[0]
        if group != prev_group:
            if prev_group:
                print("-" * 100)
            prev_group = group
